...
import maya.GUIElements as GE
import maya.Styles as style
...
class GUI(tk.Tk):
    def __init__(self, models):  
        ...
        self.models = models
        self.title('Hinduism, "Maya" (माया) refers to the concept of illusion or the deceptive nature of the material world.')
        self.target_media = []
...
####### Middle Frame  
 ...
 ######### Options
        ...
        self.status_left_label = tk.Label(bottom_frame, style.donate_1, cursor="hand2", text="linkedin account")
        self.status_left_label.grid( row = 0, column = 0, sticky='NEWS')
        self.status_left_label.bind("<Button-1>", lambda e: self.callback("https://www.linkedin.com/in/jitendarkumarsaini25/"))


        self.status_label = tk.Label(bottom_frame, style.donate_1, text="jitendra Github")
        self.status_label.grid( row = 0, column = 1, sticky='NEWS')
        self.status_label.bind("<Button-1>", lambda e: self.callback("https://github.com/jitendra205saini"))

        # self.donate_label = tk.Label(bottom_frame, style.donate_1, text="Enjoy maya? Please Support! (Paypal) ", anchor='e')
        # self.donate_label.grid( row = 0, column = 2, sticky='NEWS')
        # self.donate_label.bind("<Button-1>", lambda e: self.callback(""))
...
# refactor - make sure files are closed
    ...
    def initialize_gui( self ):
        ...
        self.video_image = cv2.cvtColor(cv2.imread('./maya/media/splash.png'), cv2.COLOR_BGR2RGB)
        self.resize_image()
